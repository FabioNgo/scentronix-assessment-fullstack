...
import { ActionMenuItem, ActionMenuItemProps } from '@/components/ActionMenu/ActionMenuItem/ActionMenuItem';
...
export function ActionMenuItemList({ items }: ActionMenuItemListProps) {
  ...
  return <StyledActionMenuItemList>
    {items.map(item => {
      return <ActionMenuItem key={`item-${item.id}`} {...item}></ActionMenuItem>;
    })}
